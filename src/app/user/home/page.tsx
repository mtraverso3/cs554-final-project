...
export default function UserHome() {
  ...
  useEffect(() => {
    setRecentDecks(dummyDecks.slice(0, 3));
    setRecentQuizzes(dummyQuizzes);
    
    // these r study stats (FOR NOW!!)
    const totalCards = dummyDecks.reduce((sum, deck) => sum + deck.flashcardCount, 0);
    const cardsStudied = dummyDecks.reduce((sum, deck) => sum + Math.round(deck.flashcardCount * deck.studyProgress / 100), 0);
    const quizzesCompleted = dummyQuizzes.reduce((sum, quiz) => sum + quiz.completedCount, 0);
    
    const completedQuizzes = dummyQuizzes.filter(q => q.lastScore !== null);
    const averageScore = completedQuizzes.length
      ? completedQuizzes.reduce((sum, q) => sum + (q.lastScore || 0), 0) / completedQuizzes.length
      : 0;
        ...
        {showStats && (
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 py-2">
              ...
              <div className="bg-primary-foreground p-4 rounded-lg text-center">
                <p className="text-2xl font-bold">{studyStats.quizzesCompleted}</p>
                <p className="text-sm text-muted-foreground">Quizzes Completed</p>
              </div>
              <div className="bg-primary-foreground p-4 rounded-lg text-center">
                <p className="text-2xl font-bold">{studyStats.averageScore.toFixed(1)}%</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {recentDecks.map(deck => (
            <Card key={deck.id} className="h-full">
              ...
              <CardContent>
                <p className="text-muted-foreground line-clamp-2 mb-3">{deck.description}</p>
              ...
              <CardFooter className="flex justify-between pt-2">
                <Button variant="default" size="sm" asChild>
                  <Link href={`/user/flashcard-library/${deck.id}/study`}>Study</Link>
                </Button>
          ...
          {recentQuizzes.map(quiz => (
            <Card key={quiz.id} className="h-full">
              <CardHeader>
                ...
                <CardDescription>
                  {quiz.flashcardCount} flashcards • Created on {quiz.createdAt.toLocaleDateString()}
                </CardDescription>
              </CardHeader>
              <CardContent>
                <p className="line-clamp-2 text-muted-foreground mb-3">{quiz.description}</p>
                ...
                {quiz.lastScore !== null ? (
                  <div className="flex items-center mb-1">
                    <div 
                      ...
                      style={{ 
                        backgroundColor: quiz.lastScore >= 80 ? '#4ade80' : 
                                         quiz.lastScore >= 60 ? '#facc15' : '#f87171',
                        width: '60px'
                      }}
                    >
                      {quiz.lastScore}%
              ...
              <CardFooter className="flex justify-between">
                ...
                <div className="space-x-2">
                  ...
                  <Button variant="ghost" size="sm" asChild>
                    <Link href={`/user/quiz-library/${quiz.id}/stats`}>Stats</Link>
                  </Button>
                </div>
              </CardFooter>
            </Card>
